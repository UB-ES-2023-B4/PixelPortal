...
def test_get_likes_by_user_id(test_client):
    ...
    for i in range(4):
        test_client.post("/likes/", json={"usuario_id":user_id, "publicacion_id": valid_pb_id_list[i]},
                                headers=headers)
...
def test_post_like(test_client, count):
    ...
    for i in range(4):
        test_client.post("/likes/", json={"usuario_id":user_id, "publicacion_id": valid_pb_id_list[i]},
                                headers=headers)
    response = test_client.get(f"/likes/user/{invalid_likes_data_list[count]['usuario_id']}")
    response_data = response.json()
    assert response.status_code != 200, response_data
    assert type(response_data) != type(list()), response_data
