...
def delete_with_payload(test_client,  **kwargs):
    return test_client.request(method="DELETE", **kwargs)
...
def test_delete_like(test_client, count):
    ...
    response = delete_with_payload(test_client,url = "/likes/", json = {"usuario_id":user_id, "publicacion_id": valid_pb_id_list[count]}, headers=headers)
