...
def test_valid_post_like(test_client, count):
...
def post_images(test_client, access_token, image_data):
    ...
    return response_data['id']
@pytest.mark.parametrize("count", [0, 1, 2, 3])
def test_invalid_post_like(test_client, count):
    access_token, user_id = login_user_post(test_client, user_data)
    valid_pb_id_list = []
    for image in image_data_list:
        valid_pb_id_list.append(post_images(test_client, access_token, image))  # Post de la imagen
    headers = {"Authorization": f"Bearer {access_token}"}
    if count == 0:
        response = test_client.post("/likes/", json={"usuario_id": invalid_likes_data_list[count]['usuario_id'], "publicacion_id": invalid_likes_data_list[count]['publicacion_id']},
                        headers=headers)
